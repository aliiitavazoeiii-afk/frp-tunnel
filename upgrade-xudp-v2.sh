...
[[ "$ROLE" == "foreign-a" || "$ROLE" == "foreign-b" || "$ROLE" == "iran" ]] || die "usage: sudo bash upgrade-xudp-v2.sh foreign-a|foreign-b|iran"
...
backup_state(){
  ...
  b="$STATE_DIR/backups/xudp-v2-${ts}-${ROLE}"
  mkdir -p "$b"
  ...
  "outbounds": [
    {
      ...
      "streamSettings": {
        "network": "raw",
        "sockopt": {"dialerProxy": "anytls-carrier"}
      },
      "mux": {
        "enabled": true,
        "concurrency": -1,
    ...
    },
    {
      ...
      "settings": {
        "servers": [
          {"address": "127.0.0.1", "port": ${LOCAL_SOCKS_PORT}, "users": []}
        ]
      }
    }
  ],
  ...
  "inbounds": [
    {
      ...
      "settings": {
        "clients": [
          {"id": "${XUDP_UUID}", "email": "anytls-xudp"}
        ],
        "decryption": "none"
      }
    }
...
JSON
  ...
  if ! validate_config "$out"; then
    cp -a "$out" "$BACKUP_DIR/failed-xudp-bridge.json" || true
    rm -f "$out"
    die "Xray bridge config validation failed; full Xray error is printed above"
  fi
...
try:
    ...
    for ob in cfg.get("outbounds",[]):
        ...
        if isinstance(st.get("servers"),list) and st["servers"]:
            ...
            st["servers"][0]["port"]=new_port
        else:
            st.clear(); st.update({"servers":[{"address":"127.0.0.1","port":new_port,"users":[]}]})
...
PY
  ...
  local runtime_port
  runtime_port=$(jq -r '.outbounds[] | select(.tag=="anytls-tunnel") | (.settings.servers[0].port // empty)' "$XUI_RUNTIME" 2>/dev/null | head -n1 || true)
...
if [[ "$role" == "iran" ]]; then
  ...
  echo "x-ui outbound:"
  jq -r '.outbounds[] | select(.tag=="anytls-tunnel") | "  tag=\(.tag) port=\(.settings.servers[0].port // "?") targetStrategy=\(.targetStrategy // "?")"' /usr/local/x-ui/bin/config.json 2>/dev/null || true
fi
EOFH
...
if [[ "$ROLE" == "iran" ]]; then
  [[ "$XUDP_SOCKS_PORT" != "$LOCAL_SOCKS_PORT" ]] || die "XUDP_SOCKS_PORT must differ from Mihomo SOCKS port"
  if ss -H -ltn "sport = :${XUDP_SOCKS_PORT}" 2>/dev/null | grep -q . && ! systemctl is-active --quiet "$BRIDGE_SERVICE" 2>/dev/null; then
    ss -ltnp "sport = :${XUDP_SOCKS_PORT}" || true
    die "TCP/${XUDP_SOCKS_PORT} already in use"
  fi
else
  if ss -H -ltn "sport = :${XUDP_SERVER_PORT}" 2>/dev/null | grep -q . && ! systemctl is-active --quiet "$BRIDGE_SERVICE" 2>/dev/null; then
