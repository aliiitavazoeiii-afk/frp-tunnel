...
TRUST_PORT=${TRUST_SPLIT_PORT:-7991}
MIERU_PORT=${MIERU_SPLIT_PORT:-7992}
...
probe_http(){
  ...
  for attempt in $(seq 1 "$attempts"); do
    ...
    ok=0
    if [[ "$expect" == 204 ]]; then [[ "$code" == 204 ]] && ok=1; else [[ "$code" =~ ^[234][0-9][0-9]$ ]] && ok=1; fi
    if (( ok == 1 )); then
      if (( attempt > 1 )); then log "$label recovered on attempt $attempt/$attempts"; fi
      log "$label = OK (HTTP=$code)"; return 0
    fi
    if (( attempt < attempts )); then log "$label transient failure HTTP=${code:-000}; retry $((attempt+1))/$attempts"; sleep "$attempt"; fi
  done
...
probe_transfer(){
  ...
  for attempt in $(seq 1 "$attempts"); do
    ...
    if [[ "$code" == 200 && "$size" =~ ^[0-9]+$ && "$size" -ge 750000 ]]; then
      if (( attempt > 1 )); then log "$label transfer recovered on attempt $attempt/$attempts"; fi
      log "$label transfer = OK (${size} bytes in ${secs}s)"; return 0
    fi
    if (( attempt < attempts )); then log "$label transfer transient failure HTTP=$code bytes=$size time=$secs; retry $((attempt+1))/$attempts"; sleep "$attempt"; fi
  done
  die "$label transfer failed after $attempts attempts HTTP=$code bytes=$size time=$secs"
}

for p in "$ENTRY_PORT" "$TRUST_PORT" "$MIERU_PORT"; do ss -H -ltn "sport = :$p" 2>/dev/null | grep -q . || die "local SOCKS TCP/$p not listening"; done

log "Forced Trust split path (TCP direct carrier)"
probe_http "$TRUST_PORT" "TRUST gstatic" https://www.gstatic.com/generate_204 204
log "Forced Mieru split path (TCP direct carrier)"
...
if [[ "$MODE" == "--quick" ]]; then log "SUCCESS: both split paths and dispatcher passed quick TCP probe"; exit 0; fi
...
log "SUCCESS: Trust + Mieru split TCP and dual UDP/XUDP full probe passed"
