...
systemctl stop x-ui
XUI_DB="$XUI_DB" INBOUND_TAG="$INBOUND_TAG" ENTRY="$ENTRY" python3 <<'PY'
import json,os,sqlite3
p=os.environ['XUI_DB']; tag=os.environ['INBOUND_TAG']; port=int(os.environ['ENTRY'])
...
try:
    ...
    if len(matches)>1: raise RuntimeError('multiple dual-tunnel outbounds found')
    desired={
...
print('RUNTIME OK: public :443 -> dual-tunnel -> 127.0.0.1:7990; sniffing ON; targetStrategy=AsIs')
PY
